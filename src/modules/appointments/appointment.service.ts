...
export class AppointmentService {
  ...
  async create(data: CreateAppointmentInput) {
    const appointmentDate = parseIsoDate(
      data.date,
      'Data do agendamento invalida.'
    );

    if (isPast(appointmentDate)) {
      throw new AppError(
        'Nao e possivel realizar um agendamento em uma data passada.',
        400
      );
    }
    ...
    const requestedEnd = addMinutes(appointmentDate, service.duracaoMinutos);

    return prisma.$transaction(async (transaction) => {
      await transaction
        .$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${data.profissionalId}))`;

      const existingAppointments = await transaction.agendamento.findMany({
        where: {
      ...
      if (conflict) {
        throw new AppError(
          'Este horario ja esta ocupado para este profissional.',
          409
        );
      }
  ...
  async findAllByUser(user: AuthenticatedUser) {
    return prisma.agendamento.findMany({
      ...
      include: appointmentHistoryInclude,
      orderBy: {
        dataHora: 'desc',
      },
    });
  ...
  async getAvailability(profissionalId: string, date: string, servicoId?: string) {
    ...
    if (servicoId) {
      const service = await prisma.servico.findFirst({
        where: {
          id: servicoId,
          barbeariaId: professional.barbeariaId,
        },
        select: {
          duracaoMinutos: true,
        },
      ...
      requestedDuration = service.duracaoMinutos;
    }
  ...
  async cancel(appointmentId: string, user: AuthenticatedUser) {
    ...
    const cancellationDeadline = subHours(appointment.dataHora, 2);

    if (isPast(cancellationDeadline)) {
      throw new AppError(
